...
const DayColumn = ({ day, id }: props) => {
  ...
  // declare main array
  let numberOfDaysArray: { value: number, id: number, month: number, current: boolean}[] = [];
  ...
  for ( let x: number = 1; x <= numberOfDaysInMonth; x++ ) {
    ...
    numberOfDaysArray.push({ value: x, id: +idNumber, month: monthValue, current: true });
  };
  ...
  for ( let x: number = 0; x > -firstDayOfMonth; x-- ) {
    ...
    numberOfDaysArray.unshift({ value: getPreviousDate, id: +idNumber, month: previousMonth, current: false});
  };
  ...
  for ( let x: number = 6; x > lastDayOfMonth; x-- ) {
    ...
    numberOfDaysArray.push({ value: getNextDate, id: +idNumber, month: nextMonth, current: false});
    inc++;
  };

  // function to assign dates to relevant day column
  const calculateDateGrid = (dateInput: number, itemId: number, month: number, current: boolean) => {
    ...
    if (weekdayNumber === id) {
        return <DayBox key={itemId} dayValue={dateInput} boxClass={current} />
      };
    };
  ...
  return (
    <li className='weekday'>
      ...
      { numberOfDaysArray.map(x => 
        calculateDateGrid(x.value, x.id, x.month, x.current)
      )}
    </li>
  );
};
